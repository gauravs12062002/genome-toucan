...
public class LaganDialog
  ...
  public LaganDialog(MainFrame window) {
    super(window, "LAGAN Web Service", true);
    this.window = window;
    Gene g;
    glVec = new Vector();
    ...
    JLabel label8 = new JLabel("P5: Reverse complement Seq2");
    ...
    try {
      if (e.getSource() == ok) {
        if (((String)soapCmb.getSelectedItem()).equalsIgnoreCase(""))
          ...
          ServiceRunner sr = new ServiceRunner(window, ServiceRunner.LAGAN);
          ...
          if (check8.isSelected())sr.setRev(1);
          Gene g1 = (Gene) window.gl.list.get(geneCmb1.getSelectedIndex());
          Gene g2 = (Gene) window.gl.list.get(geneCmb2.getSelectedIndex());
