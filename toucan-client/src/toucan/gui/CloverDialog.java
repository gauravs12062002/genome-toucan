...
/**
 * Title: CloverDialog
 * Description: Dialog to run the Clover service
...
public class CloverDialog
  ...
  public CloverDialog(MainFrame window) {
    super(window, "Clover Web Service", true);
    owner = window;
    ...
    JLabel label7 = new JLabel("P3: Background sequences * (String)");
    JLabel label6 = new JLabel("P4: P-value threshold (double)");
    JLabel label8 = new JLabel("P5: Number of randomizations");
    ...
    txt6 = new JTextField("0.01", 20); //threshold
    txt7 = new JTextField("Choose get or browse", 20); //bgModel
    txt7.setEditable(false);
    txt8 = new JTextField("10",20); //number of randomizations
    ...
    GridLayout lay = new GridLayout(0, 2, 1, 0);
    ...
    this.setSize(owner.getWidth()/2,owner.getHeight()/4);
    this.pack();
    ...
    try {
      if (e.getSource() == ok) {
        if (txt5.getText().equalsIgnoreCase("") ||
            txt7.getText().equalsIgnoreCase("") ||
            txt5.getText().equalsIgnoreCase("Choose get or browse") ||
            txt7.getText().equalsIgnoreCase("Choose get or browse")) {
          ...
          if (!txt6.getText().equals("")) {
            sr.setcThreshold(new Double(txt6.getText()).
                        doubleValue());
          }
          if (!txt8.getText().equals("")) {
              sr.setNrRand(new Integer(txt8.getText()).
                          intValue());
            }
          if (!txt9.getText().equals("")) {
            sr.setStrand(new Integer(txt9.getText()).
                         intValue());
          }
...
//          msg[2] =               "If you close the Toucan application, the results will be lost";
  ...
  public void getParams(String soapUrl) throws Exception {
    try {
      //MotifScannerClient msc = new MotifScannerClient(window,txt0.getText(),txt1.getText(),txt2.getText(),"getParams");
      ...
      for (int i = 0; i < nl.getLength(); i++) {
        n = nl.item(i);
        map = n.getAttributes();
        if (map.getNamedItem("type").getNodeValue().equalsIgnoreCase("fasta")) {
