...
/**
 * Title: Stats
 * Description: Dialog to run a binomial analysis to find the over-represented features (motifs)
...
public class Stats extends JFrame implements ClipboardOwner{
  ...
  JCheckBox chkSub = new JCheckBox("Use sublist size");
  ...
  private void jbInit() throws Exception {
    ...
    /*{
     public Component prepareRenderer(TableCellRenderer renderer,
            ...
            if (Stats.this.replacedFreqs.contains(new Integer(rowIndex))) {
    ...
    refFileButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e){
                int returnVal = fc.showOpenDialog(Stats.this);
    ...
    expFreqFileButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e){
                int returnVal = fc2.showOpenDialog(Stats.this);
                if (returnVal == JFileChooser.APPROVE_OPTION) {
	...
	buf.append("\n");
     ...
     clipboard.setContents(sel,Stats.this);
    }
    });
    pop.add(copyItem);
    ...
    startButton.addActionListener(new ActionListener() {
		   ...
		   if(!refFile.getText().equalsIgnoreCase("") && !expFreqFile.getText().equalsIgnoreCase("")){
		         JOptionPane.showMessageDialog(Stats.this,"Choose either one or another","Error",JOptionPane.ERROR_MESSAGE);
		   }
		   else if (refFile.getText().equalsIgnoreCase("") && expFreqFile.getText().equalsIgnoreCase("")){
		         JOptionPane.showMessageDialog(Stats.this,"Specify a file for the expected frequencies","Error",JOptionPane.ERROR_MESSAGE);
		   }
		   else if (!refFile.getText().equalsIgnoreCase("")){
			try{
			  objects = owner.gl.binom(refFile.getText(),false,-1);
			}
			catch(Exception exc){
			  exc.printStackTrace();
			  JOptionPane.showMessageDialog(Stats.this,"There were problems with the Binomial Analysis...","Error",JOptionPane.ERROR_MESSAGE);
			}
		   }
		   else if (!expFreqFile.getText().equalsIgnoreCase("")){
		         try{
                           if(chkSub.isSelected()){
                             if(owner.subList.list.size()==0) JOptionPane.showMessageDialog(Stats.this,"Your sublist is empty!","Error",JOptionPane.ERROR_MESSAGE);
                             System.out.println("sublist size=" + owner.subList.getT());
                             objects = owner.gl.binom(expFreqFile.getText(),true,owner.subList.getT());
                             //objects = owner.subList.binom(expFreqFile.getText(),true,-1);
                           }
                           else objects = owner.gl.binom(expFreqFile.getText(),true,-1);
                         }
			 catch(Exception exc){
			  exc.printStackTrace();
			  JOptionPane.showMessageDialog(Stats.this,"There were problems with the Binomial Analysis...","Error",JOptionPane.ERROR_MESSAGE);
		   ...
		   if(objects!=null){
			  ...
			  if(!o.freqReplaced){
		       ...
		       Stats.this.sortAllRowsBy(model,3,false);
		   }
	    }
    });
